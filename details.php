...
if(isset($_POST['details_connect'])){
  ...
  if(!empty($email)&&!empty($password_hash)){
    ...
    echo "Error in query.. <br />";
      ...
      if(@($email==$email_database)&&($password_hash==$password_database_hash)){
        //require 'profile.php';
        header("Location:details.php?pro_id=$pro_id&q=".$email_database);
      } else {
...
if(isset($_POST['details_signup_connect'])){
  ...
  if(!empty($first_name)&&!empty($mobile_no)&&!empty($email)&&!empty($password)&&!empty($password_again)){
    if($password_hash!=$password_again_hash){
      ...
      if($query_run=mysql_query($query)){
        ...
        echo  "<script>window.open('details.php?pro_id=$pro_id&q=$email','_self')</script>";
...
<head>
  ...
  $(function(){
  $("#details_hover").click(function(){
    $("#details_hover_overlay").fadeIn();
    $("#details_hover_img").fadeIn();
  });
});
$(function(){
  $("#zoom_pic").mouseover(function(){
    $("#zoom_image").fadeIn('slow');
    $("#zoom_overlay").fadeIn('slow');
  });
  $("#zoom_pic").mouseout(function(){
    $("#zoom_image").fadeOut('slow');
    $("#zoom_overlay").fadeOut('slow');
  });
});
  </script>
</head>
            ...
            <div id="inner_login_header1">
              ...
              <div id="inner_login_form">
                <form action="" method="post">
                  ...
                  <font id="submit_right"><input type="submit" name="details_signup_connect" value="Sign Up" /></font>
                </div>
              </div>
            </div>
          </li>
              ...
              <div id="inner_login_header">
                ...
                <div id="inner_login_form">
                  <form action="" method="post">
                    ...
                    <font id="forget_pass"><a href="">forget Password?</a></font><br />
                    <font id="submit_right"><input type="submit" name="details_connect" value="Sign In" /></font>
                  </div>
                </div>
              </div>
            </li>
        ...
        <div class="product_insert_box">
            ...
            if(isset($_GET['pro_id'])){
              ...
              while($row_pro=mysqli_fetch_array($run_pro)){
                ...
                <div id='inner_insert_product_details'>
                <table id='details_table' width='670px'>
                <tr>
                  <td>
                     <h3><font id='pro_title'>$pro_title</font></h3>
                  </td>
                 <tr>
                   <td width='50px'>
                     <div id='zoom_pic'><img src='admin_area/product_images/$pro_image' width='250' height='370' float:center; /></div>
                     ...
                     <br /><font id='pro_desc'>$pro_desc</font><br /><br />
                  </td>
                  <td width='350px'>

                    <font id='related_pro'>Related Products :</font><br /><br />
                ...
                <font id='go_back'><a href='home.php'>Go Back</a></font>
            ...
            <div id="footer_pro">
              <div id="banner_header">
                   <font id="banner_1">Related Products :</font>
                </div>
                <br />
                <?php
                $get_pro_h="select * from products order by RAND() LIMIT 0,1";
                $run_pro_h=mysqli_query($con,$get_pro_h);
                while($row_pro=mysqli_fetch_array($run_pro_h)){

                  $pro_id_h=$row_pro['product_id'];
                  $pro_cat_h=$row_pro['product_cat'];
                  $pro_brand_h=$row_pro['product_brand'];
                  $pro_title_h=$row_pro['product_title'];
                  $pro_price_h=$row_pro['product_price'];
                  $pro_image_h=$row_pro['product_image'];
                  $pro_desc_h=$row_pro['product_desc'];

                }
                ?>
                <table width="700px" bgcolor="sienna" style="border-radius:4px;">
                  <tr>
                    <td>
                      <?php
                      echo "<img src='admin_area/product_images/$pro_image_h' width='80px' height='80px' />";
                        ?>
                    </td>
                    <td>
                    <?php
                    echo "
                    <font id='pro_title_header'>$pro_title_h</font><br />
                    <font id='pro_desc_header'>$pro_desc_h</font>
                    ";
                    ?>
                    </td>
                    <td>
                    <?php
                    echo "
                    <font id='pro_price_header'>Price :$ $pro_price_h </font><br />
                    <font id='submit_cart_header'><a href='home.php?add_cart=$pro_id_h'><input type='button' value='Add to Cart' ></a></font>
                    ";
                    ?>
                    </td>
                  </tr>
...
<!--
  <div id="details_hover_overlay">
  <div id="hover_img">
    <?php
  /*  echo "
    <img src='admin_area/product_images/$pro_image' width='250' height='370' />
    ";*/
    ?>
  </div>
</div>
